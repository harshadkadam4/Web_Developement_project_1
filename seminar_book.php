...
if ($date2->invert == 1) {
	if (preg_match("/^[a-zA-z]*$/", $fname)) 
	{
		...
		//$sql="select date from seminar_book";
		$sql = "select date from seminar_book where date='$date' and status='$status' and time='$time'";
		$result = $conn->query($sql);

		if ($result->rowCount() > 0) {
			//echo "<script>alert('Booking Successful');</script>";
			echo "<script>alert('Hall is Already Booked on your entered Date or Time .Please enter another Date or Time');
	window.location.href = 'seminar_book.html';</script>";
				} else {
					$sql1 = "insert into seminar_book(name,contact,date,dept,status,last_name,time) values('$fname','$contact','$date','$dept','$status','$lname','$time');";
					$result1 = $conn->query($sql1);
					if ($result1->rowCount() > 0) {

						$sql2 = "select book_id from seminar_book where date='$date' and time='$time'";
